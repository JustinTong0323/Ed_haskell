...
-- any one from NoRep : ok
-- one from comp and one from norep (corresponding) : too many wrong sols
-- one from comp and one from norep (not corresponding) : ok (342.68 secs, 242,432,109,624 bytes)
...
toLiterals (And clauses) = concat $ map unpack clauses
  where
    unpack (Or literals) = literals

showSquares :: [Literal (Int, Int, Int)] -> String
showSquares lits =
  let pos = [a | P a <- lits]
   in [ (intToDigit . last)
          [ k | k <- [0 .. 9], (i, j, k) `elem` pos || k == 0
          ]
        | i <- [1 .. 9],
          j <- [1 .. 9]
      ]
